...
class JudgeWordsController extends Controller
{
    public function hasViolations($check_word){
        ...
        {
            $check = in_array($check_word->hint1,$blacklist_words,true);
            if($check)
            {
                Violations::create([
                    'uid'=>$check_word->judge_id,
                ...
                $check = in_array($check_word->hint2,$blacklist_words,true);
                if($check)
                {
                    Violations::create([
                        'uid'=>$check_word->judge_id,
                    ...
                    $check = in_array($check_word->hint3,$blacklist_words,true);
                    if($check)
                    {
                        Violations::create([
                            'uid'=>$check_word->judge_id,
    ...
    public function store(Request $request){
        ...
        // if($foundViolation){
        //     Words::update('status','violation')->where('id',$new->id);
        // }
        return $new;
    }
